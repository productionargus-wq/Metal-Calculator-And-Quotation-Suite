...
app.post('/api/org/profile', async (req, res) => {
  try {
    ...
    if (typeof brevoApiKey === 'string') {
      org.brevoApiKey = brevoApiKey.trim();
    }
...
// Send Quotation PDF via Email Route (Resend / Brevo / SMTP)
app.post('/api/quote/send-email', async (req, res) => {
  try {
    const { recipientEmail, clientName, subject, notes, pdfBase64, pdfFilename, orgName, isWithWorkings } = req.body;
    if (!recipientEmail) {
    ...
    // Fetch Organisation record if present
    let org = null;
    if (cleanOrgName) {
      org = await Organisation.findOne({ name: cleanOrgName });
    }

    const cleanBase64 = (pdfBase64 || '').replace(/^data:application\/pdf;base64,/, '');
    const emailSubject = subject || `Commercial Quotation from ${cleanOrgName || 'Argus Quotation Suite'}`;
    const emailHtml = `
    ...
    // 1. Dispatch via RESEND Cloud API (If configured in org profile or .env)
    const resendKey = (org && org.resendApiKey) || process.env.RESEND_API_KEY;
    if (resendKey && resendKey.trim()) {
      try {
        ...
        if (resendRes.ok && resendData.id) {
          console.log(`[Quote Mail] Resend delivery successful: ${resendData.id}`);
          return res.status(200).json({
            success: true,
            provider: 'resend',
          ...
          console.warn('[Quote Mail] Resend API error:', resendData);
          throw new Error(resendData.message || resendData.error || 'Resend API returned an error.');
        }
      } catch (resendErr) {
        console.error('[Quote Mail] Resend Error:', resendErr);
        if (!org?.brevoApiKey && !process.env.BREVO_API_KEY && !org?.smtpPass && !process.env.SMTP_PASS) {
          return res.status(502).json({ error: `Resend delivery failed: ${resendErr.message}` });
        }
      }
    }

    // 2. Dispatch via BREVO Cloud API (If configured in org profile or .env)
    const brevoKey = (org && org.brevoApiKey) || process.env.BREVO_API_KEY;
    if (brevoKey && brevoKey.trim()) {
      try {
        console.log(`[Quote Mail] Sending via Brevo Cloud API to ${cleanRecipient}`);
        const senderEmail = process.env.BREVO_FROM_EMAIL || (org && org.email) || 'quotes@arguscnc.com';
        const senderName = process.env.BREVO_FROM_NAME || cleanOrgName || 'Argus Quotation Suite';

        const brevoPayload = {
          sender: { name: senderName, email: senderEmail },
          to: [{ email: cleanRecipient, name: clientName || 'Client' }],
          subject: emailSubject,
          htmlContent: emailHtml,
          attachment: cleanBase64 ? [
            {
              name: pdfFilename || 'Commercial_Quotation.pdf',
              content: cleanBase64
            }
          ] : []
        };
        if (org && org.email) {
          brevoPayload.replyTo = { email: org.email, name: cleanOrgName };
        }
        ...
        if (brevoRes.ok) {
          console.log(`[Quote Mail] Brevo delivery successful: ${brevoData.messageId}`);
          return res.status(200).json({
            success: true,
            provider: 'brevo',
            message: `Quotation PDF sent successfully to ${cleanRecipient}`,
          ...
          console.warn('[Quote Mail] Brevo API error:', brevoData);
          throw new Error(brevoData.message || 'Brevo API returned an error.');
        }
      } catch (brevoErr) {
        console.error('[Quote Mail] Brevo Error:', brevoErr);
        if (!org?.smtpPass && !process.env.SMTP_PASS) {
          return res.status(502).json({ error: `Brevo delivery failed: ${brevoErr.message}` });
        }
      }
    }

    // 3. Fallback: Dispatch via SMTP (Nodemailer)
    ...
    if (smtpUser && smtpPass) {
      ...
      console.log(`[Quote Mail] SMTP message sent to ${cleanRecipient}: ${info.messageId}`);
      return res.status(200).json({
        success: true,
        provider: 'smtp',
        message: `Quotation PDF sent successfully to ${cleanRecipient}`,
    ...
    return res.status(502).json({
      error: `Email delivery failed: ${err.message || 'Please verify your email credentials in Settings.'}`
    });
  }
});
